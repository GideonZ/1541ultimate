...
JSON *Assembly :: send_query(const char *query)
{
    ...
    int n = send(this->socket, request.c_str(), request.length(), MSG_DONTWAIT);
    get_response(collect_in_buffer);
    
    JSON *json = convert_buffer_to_json((t_BufferedBody *)response.userContext);
    return json;
}
...
int Assembly :: read_socket(void)
{
    ...
    printf("Recv %d -> %p\n", space, p);
    int n = space ? recv(this->socket, p, space, 0) : 0;
    if (n >= 0) {
        response._valid += n;
    }
    return n;

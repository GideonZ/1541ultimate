...
API_DOC(GET, machine, input,
    ...
    DESCRIPTION("Returns every key and joystick input that is held right now, both the ones this "
                "API is holding and the ones the Ultimate menu is holding, so the answer matches "
                "what the machine sees. `mouse` covers only the mouse this API drives; its "
                "buttons reach control port 1 but are not listed under `joysticks`.\n"
                "\n"
...
API_DOC(POST, machine, input,
    ...
    DESCRIPTION("Applies up to 64 input events in the order they are given and returns the state "
                ...
                "and `tap` does both. A `release_all` event drops everything this API is holding, "
                "and so does a machine reset. `restore` is not part of the keyboard matrix but "
                "wired to NMI, so only `tap` does anything with it.\n"
                "\n"
                "A `mouse` event presses, releases or taps buttons, sends one `move`, turns the "
                "`wheel` by whole detents, or replays a `path` of moves. The firmware handles the "
                "reports as it handles a USB mouse's, sending them in order at least 20ms apart "
                "and pacing the movement so the pointer never jumps backward. `mouse.pending` "

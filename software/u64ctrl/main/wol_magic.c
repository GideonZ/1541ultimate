...
{
    // No station owns either, and both would turn a run of padding into a wake
    // request: broadcast is the sync stream itself, all-zero is a zeroed frame.
...
bool wol_is_magic_packet(const uint8_t *frame, size_t len, const uint8_t *mac)
{
    ...
    for (size_t at = 0; (at + WOL_PATTERN_LEN) <= len; at++) {
        ...
        // Every offset is tried, not just the first 0xFF found, so a frame
        // that starts with more than six 0xFF bytes still matches.
        int repeats = 0;

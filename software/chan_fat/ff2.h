...
typedef struct {
	...
	DRVREF	drv;			/* Physical drive reference */
...
FRESULT f_mount (FATFS* fs, const TCHAR* path, DRVREF pdrv, BYTE opt);	/* Mount/Unmount a logical drive */
FRESULT f_mkfs (const TCHAR* path, BYTE sfd, UINT au);				/* Create a file system on the volume */
FRESULT f_fdisk (DRVREF pdrv, const DWORD szt[], void* work);		/* Divide a physical drive into some partitions */
...
FRESULT fs_unlink (FATFS *fs, const TCHAR* path);								/* Delete an existing file or directory */
FRESULT fs_rename (FATFS *fs, const TCHAR* path_old, const TCHAR* path_new);	/* Rename/Move a file or directory */
FRESULT fs_stat (FATFS *fs, const TCHAR* path, FILINFO* fno);					/* Get file status */
FRESULT fs_chmod (FATFS *fs, const TCHAR* path, BYTE attr, BYTE mask);			/* Change attribute of the file/dir */
FRESULT fs_utime (FATFS *fs, const TCHAR* path, const FILINFO* fno);			/* Change times-tamp of the file/dir */
FRESULT fs_getfree (FATFS *fs, DWORD* nclst);									/* Get number of free clusters on the drive */

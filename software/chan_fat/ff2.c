...
#error Wrong include file (ff2.h).
#endif
...
#define UNLOCK_FF(fs, res)  { unlock_fs(fs, res); }
#else
#define	ENTER_FF(fs)
#define LEAVE_FF(fs, res)	return res
#define UNLOCK_FF(fs, res)
#endif
...
// GZW: This should be a separate function, available externally
...
FRESULT f_mount (
	...
	DRVREF pdrv,		/* Reference to the drive that is being mounted */
...
)
{
	...
	fs->drv = pdrv;						/* Link drive to fs object */
...
)
{
	...
	res = find_volume(&fs, &path, (BYTE)(mode & ~FA_READ));
#else
	mode &= FA_READ;
	...
	if (res == FR_OK) {
		return fs_open(fs, path, mode, fp); // will lock again
	}
	return res;
}
...
)
{
	...
	/* Get logical drive number */
	FRESULT res = find_volume(&fs, &path, 1);
	if (res != FR_OK)
		return res;
...
)
{
	...
	if (res != FR_OK)
		return res;
	return fs_chmod(fs, path, attr, mask);
}
...
)
{
	...
	FRESULT res = find_volume(&fs, &path_old, 1);
	if (res != FR_OK)
		return res;
	return fs_rename(fs, path_old, path_new);
}
...
)
{
	...
	if (res != FR_OK)
		return res;
	return fs_getlabel(fs, label, vsn);
}
...
)
{
	...
	/* Get logical drive number */
	FRESULT res = find_volume(&fs, &label, 1);
	if (res != FR_OK)
		return res;
	return fs_setlabel(fs, label);
}

...
// tester instance
FactoryRegistrator<BrowsableDirEntry *, FileType *> tester_ddl(FileType :: getFileTypeFactory(), FileTypeDDL :: test_type);
...
FileTypeDDL :: FileTypeDDL(BrowsableDirEntry *n, bool hdr)
{
    node = n;
    has_header = hdr;
}

FileTypeDDL :: ~FileTypeDDL()
{
}

int FileTypeDDL :: fetch_context_items(IndexedList<Action *> &list)
{
	...
	list.append(new Action("Show Image",  FileTypeDDL :: execute_st, PRGFILE_RUN, 0)); //mode));
	count = 1;
...
// static member
FileType *FileTypeDDL :: test_type(BrowsableDirEntry *obj)
{
	FileInfo *inf = obj->getInfo();
    if(strcmp(inf->extension, "DDL")==0)
        return new FileTypeDDL(obj, false);
    else
        return NULL;
}

bool FileTypeDDL :: check_header(File *f, bool has_header)
{
    return false;        
}

int FileTypeDDL :: execute_st(SubsysCommand *cmd)
{
	printf("DDL Select: %4x\n", cmd->functionID);
	File *filePlugin = 0;
    File *fileData = 0;
    ...
    FRESULT fdatares = fm->fopen(cmd->path.c_str(), cmd->filename.c_str(), FA_READ, &fileData);
    ...
    if(fileData) {
        ...
        FRESULT fpluginres = fm->fopen(pluginsPath, plugin, FA_READ, &filePlugin);

        if(filePlugin) {
        
            // No header needed for this type
            /*tmpmode = C64_MODE;
            C64_MODE = MODE_NORMAL;
        ...
        {
            cmd->user_interface->popup("DDL plugin not found.", BUTTON_OK);
        }

    } else {
        printf("Error opening file. %s\n", FileSystem :: get_error_string(fdatares));
        return -2;
    }
    return 0;
}

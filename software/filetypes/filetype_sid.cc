...
extern uint8_t _musplayer_bin_start[8192];
extern uint8_t _musplayer_bin_end;
extern uint8_t _basic_bin_start[8192];

// tester instance
FactoryRegistrator<BrowsableDirEntry *, FileType *> tester_sid(FileType ::getFileTypeFactory(),
                                                               FileTypeSID ::test_type);
...
const uint8_t petscii[59] = {'A', 'A', 'A', 'A', 'A', 'A', 'A', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'E',
                             'E', 'E', 'E', 'e', 'e', 'e', 'e', 'I', 'I', 'I', 'I', 'i', 'i', 'i', 'i',
                             'O', 'O', 'O', 'O', 'O', 'O', 'o', 'o', 'o', 'o', 'o', 'o', 'U', 'U', 'U',
                             'U', 'u', 'u', 'u', 'u', 'C', 'c', 'N', 'n', 'Y', 'Y', 'y', 'y', 'B'};
...
FileType *FileTypeSID ::test_type(BrowsableDirEntry *obj)
{
    ...
    if ((strcmp(inf->extension, "SID") == 0) || (strcmp(inf->extension, "MUS") == 0) ||
        (strcmp(inf->extension, "STR") == 0))
...
void FileTypeSID ::readSongLengths(void)
{
    ...
    if (fm->fopen(path_string.c_str(), ssl_filename.c_str(), FA_READ, &sslFile) == FR_OK) {
        ...
        if (sid_header[0x04] == 2) { // check if version of header is 2 (little endian)
            ...
            if ((sid_header[0x7B] < 0xA0) && ((sid_header[0x7B] & 0x0F) < 0x0A) && (sid_header[0x7A] < 0xA0) &&
                ((sid_header[0x7A] & 0x0F) < 0x0A)) {
    ...
    for (int i = 0; i < number_of_songs * 2; i = i + 2) {
        ...
        if ((minutes > 0x99) || ((minutes & 0x0F) > 0x09) || (seconds > 0x59) || ((seconds & 0x0F) > 0x09)) {
            sid_rom[0x3000 + i] = 0;
            sid_rom[0x3001 + i] = 0;
            minutes = 0;
            seconds = 0;
...
void FileTypeSID ::configureMusEnv(int offsetLoadEnd)
{
    ...
    int musTextOffset = start + ((dest[start + 1] + dest[start + 3] + dest[start + 5]) << 8) + dest[start] +
                        dest[start + 2] + dest[start + 4] + 6;

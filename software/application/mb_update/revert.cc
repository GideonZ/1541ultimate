...
extern uint8_t _binary_fpga_bin_start;
extern uint8_t _binary_fpga_bin_end;
...
bool program_flash(bool do_update1, bool do_update2, bool do_roms)
{
	...
	if(do_update1) {
        ...
        do {
            switch(fpga_type) {
                case 0:
        	        ok = flash_buffer(FLASH_ID_BOOTFPGA, &_binary_fpga_bin_start, &_binary_fpga_bin_end, FPGA_VERSION, "FPGA");
        	        break;
#if _FPGA400 == 1
        	    case 1:
...
int main()
{
    ...
    if(calc_checksum(&_binary_fpga_bin_start, &_binary_fpga_bin_end) == CHK_fpga_bin)

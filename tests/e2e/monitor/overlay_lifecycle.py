#!/usr/bin/env python3
"""Local-UI (overlay/freeze) debug lifecycle driver over production REST APIs.

Provides the monitor navigation primitives the local-UI suites share:
interface-type selection, monitor readiness, bank selection, address navigation,
and breakpoint set/clear. `monitor_debug_matrix_test.py` drives its overlay and
freeze cells through these helpers.

Run directly to exercise one full lifecycle per iteration:
  enter monitor -> debug step -> set bp -> continue to bp -> continue full speed
  -> reset -> re-enter monitor -> re-enter debug -> debug step
with a network-liveness probe after each iteration. Uses only shipped REST
